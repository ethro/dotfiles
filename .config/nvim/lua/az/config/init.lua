...
function M.json.load()
  local path = vim.fn.stdpath("az.config") .. "/az.json"
...
function M.load(name)
  ...
  if M.defaults[name] or name == "options" then
    _load("az.config." .. name)
  end
  _load("az.config." .. name)
  if vim.bo.filetype == "lazy" then
    -- HACK: Lazy may have overwritten options of the Lazy ui, so reset this here
    vim.cmd([[do VimResized]])
  end
  local pattern = "Lazy" .. name:sub(1, 1):upper() .. name:sub(2)
...
function M.init()
  ...
  M.did_init = true
  local plugin = require("lazy.core.config").spec.az.plugins.lazy
  if plugin then
