...
M.deprecated_extras = {
  ["az.plugins.extras.formatting.conform"] = "`conform.nvim` is now the default **az** formatter.",
  ["az.plugins.extras.linting.nvim-lint"] = "`nvim-lint` is now the default **az** linter.",
  ["az.plugins.extras.ui.dashboard"] = "`dashboard.nvim` is now the default **az** starter.",
}
...
function M.fix_imports()
  Plugin.Spec.import = Util.inject.args(Plugin.Spec.import, function(_, spec)
    ...
    if dep then
      ...
      Util.warn(dep, { title = "Lazy", once = true, stacktrace = true, stacklevel = 6 })
      return false
    end
  end)
end
